...
const VistaAlumno = (function () {
  ...
  function etiquetaEje(nombre) {
    ...
    /* Los parciales se abrevian como en la boleta: P1, P2, P3. */
    var p = t.match(/^(primer|segundo|tercer|cuarto|quinto)\s+parcial/i);
    if (p) return 'P' + ({ primer: 1, segundo: 2, tercer: 3, cuarto: 4, quinto: 5 }[p[1].toLowerCase()]);
    var corte = t.indexOf(' y ');
    if (corte > 0) t = t.slice(0, corte);
    if (t.length > 9) {
      var esp = t.lastIndexOf(' ', 9);
      t = esp > 3 ? t.slice(0, esp) : t.slice(0, 9);
    }
    return t;
  }
